...
class Maze:
    ...
    def __get_unvisited_neighbors(self, point):
        ...
        for offsets in [(0, -1), (0, 1), (1, 0), (-1, 0)]:
            ...
            if 0 <= y_ind < len(self.maze):
                if 0 <= x_ind < len(self.maze[0]):
    ...
    def __initialize_image(self):
        image = np.full((len(self.maze) * self.block_size, len(self.maze[0]) * self.block_size, 3),
                        self.BACKGROUND_COLOR_INT, dtype=np.uint8)
        return image
